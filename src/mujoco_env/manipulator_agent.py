...
from utils.util import (
    ...
    meters2xyz,
    compute_view_params,
    get_interp_const_vel_traj,
    sample_xyzs,
    get_rotation_matrix_from_two_points,
    r2quat,
    downsample_pointcloud,
)
from utils.util_preference import extract_info, is_stacked
from utils.util_visualize import find_closest_objects_3d, get_closest_pairs_img
...
class ManipulatorAgent(object):
    ...
    def __init__(self, name="Robot", rel_xml_path=None, VERBOSE=True, MODE="window"):
        """
        ...
        self.DONE = False
        self.idxs_forward = [
            self.model.joint(joint_name).qposadr[0]
            for joint_name in self.rev_joint_names[:6]
        ]
        self.idxs_jacobian = [
            self.model.joint(joint_name).dofadr[0]
            for joint_name in self.rev_joint_names[:6]
        ]
    ...
    def _parse_xml(self):
        ...
        self.full_xml_path = os.path.join(self.parent_dir, self.rel_xml_path)
        ...
        self.geom_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_GEOM, x)
        ...
        for ctrl_idx in range(self.n_ctrl):
            transmission_idx = self.model.actuator(
                self.ctrl_names[ctrl_idx]
            ).trnid  # transmission index
            joint_idx = self.model.jnt_qposadr[transmission_idx][
                0
        ...
        for ctrl_idx in range(self.n_ctrl):
            ...
            joint_idx = self.model.jnt_dofadr[transmission_idx][
                0
            ]  # index of the joint when the actuator acts on a joint
        ...
        self.sensor_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_SENSOR, x)
            for x in range(self.n_sensor)
        ]
        # Site (sites are where sensors usually located)
        self.n_site = self.model.nsite
        self.site_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_SITE, x)
    ...
    def move_body(self, body_name, pos):

        body_index = self.body_names.index(body_name)

        if body_index > -1:
            # self.model.body_pos[body_index][0] = pos[0]
            # self.model.body_pos[body_index][1] = pos[1]
            # self.model.body_pos[body_index][2] = pos[2]
            self.data.xpos[body_index][0] = pos[0]
            self.data.xpos[body_index][1] = pos[1]
            self.data.xpos[body_index][2] = pos[2]
            print(self.model.body_pos[body_index])
    ...
    def merge(self, others, merge_body="default"):
        ...
        for idx, other in enumerate(others):
            if not isinstance(other, ManipulatorAgent):
                raise XMLError(
                    "{} is not a ManipulatorAgent instance.".format(type(other))
                )
    ...
    def init_viewer(
        ...
        if MODE == "window":
            ...
            self.viewer = mujoco_viewer.MujocoViewer(
                self.model, self.data, mode="offscreen"
            )  # ,
    ...
    def update_viewer(
        ...
        if VIS_TRANSPARENT is not None:
            self.viewer.vopt.flags[mujoco.mjtVisFlag.mjVIS_TRANSPARENT] = (
                VIS_TRANSPARENT
            )
        if VIS_CONTACTPOINT is not None:
            self.viewer.vopt.flags[mujoco.mjtVisFlag.mjVIS_CONTACTPOINT] = (
                VIS_CONTACTPOINT
            )
        ...
        if CALL_MUJOCO_FUNC:
            ...
            mujoco.mjv_updateScene(
                ...
                self.viewer.pert,
                self.viewer.cam,
                mujoco._enums.mjtCatBit.mjCAT_ALL.value,
                self.viewer.scn,
            )
    ...
    def update_font_scale_from_cam_dist(
        self,
        cam_dists=[2.0, 2.5, 3.0, 4.0],
        font_scales=[300, 250, 200, 150, 100],
        VERBOSE=False,
    ):
        """
        ...
        font_scale_new = map_x_to_output(
            numbers=cam_dists, outputs=font_scales, x=cam_dist
        )
        ...
        if np.abs(font_scale_curr - font_scale_new) > 1.0:  # if font scale changes
            ...
            if VERBOSE:
                print(
                    "font_scale modified. [%d]=>[%d]"
                    % (font_scale_curr, font_scale_new)
                )
    ...
    def get_viewer_cam_info(self, VERBOSE=False):
        ...
        if VERBOSE:
            print(
                "cam_azimuth:[%.2f] cam_distance:[%.2f] cam_elevation:[%.2f] cam_lookat:%s]"
    ...
    def grab_image(
        self, resize_rate=None, interpolation=cv2.INTER_NEAREST, depth=False
    ):
        """
        ...
        if depth:
            depth_img = np.zeros(
                (self.viewer.viewport.height, self.viewer.viewport.width),
                dtype=np.float32,
            )
        mujoco.mjr_render(self.viewer.viewport, self.viewer.scn, self.viewer.ctx)
        if depth:
    ...
    def get_ik_ingredients(
        self, body_name, p_trgt=None, R_trgt=None, IK_P=True, IK_R=True, w_weight=1
    ):
        """
        Get IK ingredients
        """
    ...
    def get_ik_ingredients_geom(
        self, geom_name, p_trgt=None, R_trgt=None, IK_P=True, IK_R=True
    ):
        """
        Get IK ingredients
        """
        ...
        while True:
            ...
            if DO_RENDER:
                if ((tick - 1) % render_every) == 0:
                    ...
                    self.plot_T(
                        p=p_tcp, R=R_tcp, PLOT_AXIS=True, axis_len=0.1, axis_width=0.005
                    )
                    self.plot_T(
                        p=p_trgt,
                        R=R_trgt,
    ...
    def solve_ik_realtime(
        ...
        mujoco.mju_mulQuat(
            error_quat,
            r2quat(R_trgt @ rpy2r(np.radians([-180, 0, 90]))),
            target_quat_conj,
        )
        dw = np.zeros(3)
        mujoco.mju_quat2Vel(dw, error_quat, 1.0)
        ...
        mujoco.mj_jacBody(
            self.model, self.data, jac[3:], jac[:3], self.model.body(body_name).id
        )
        ...
        while True:
            ...
            p_contacts, f_contacts, geom1s, geom2s, body1s, body2s = (
                self.get_contact_info(
                    must_include_prefix=inc_prefix, must_exclude_prefix=exc_prefix
                )
            )
            ...
            body1s_ = [
                ...
                if obj_
                not in [
                    "ur_rg2_gripper_finger1_finger_tip_link",
                    "ur_rg2_gripper_finger2_finger_tip_link",
                ]
            ]
            body2s_ = [
                obj_
            ...
            if DO_RENDER:
                ...
                if ((tick - 1) % render_every) == 0:
                    ...
                    self.plot_sphere(
                        ...
                        r=0.005,
                        rgba=[0, 1, 0, 0],
                        label=f"self Tick: [{self.tick:.4f}]",
                    )
                    self.plot_sphere(
                        p=np.array([0, 0, 1.2]),
                        r=0.005,
                        rgba=[0, 1, 0, 0],
                        label=f"Time: [{self.tick * 0.002:.4f}]",
                    )
                    self.plot_T(
                        p=p_tcp, R=R_tcp, PLOT_AXIS=True, axis_len=0.1, axis_width=0.005
                    )
    ...
    def plot_visual_capsule(self, point1, point2, radius, rgba=[1, 1, 1, 1], label=""):
        ...
        mujoco.mjv_initGeom(
            self.viewer.scn.geoms[self.viewer.scn.ngeom - 1],
    ...
    def plot_joint_axis(self, axis_len=0.1, axis_r=0.01):
        ...
        for rev_joint_idx, rev_joint_name in zip(
            self.rev_joint_idxs, self.rev_joint_names
        ):
    ...
    def get_contact_info(self, must_include_prefix=None, must_exclude_prefix=None):
        ...
        for c_idx in range(self.data.ncon):
            ...
            if must_include_prefix is not None:
                if (
                    contact_body1[: len(must_include_prefix)] == must_include_prefix
                ) or (contact_body2[: len(must_include_prefix)] == must_include_prefix):
                ...
                if (
                    contact_body1[: len(must_exclude_prefix)] != must_exclude_prefix
                ) and (
                    contact_body2[: len(must_exclude_prefix)] != must_exclude_prefix
                ):
                    p_contacts.append(p_contact)
                    f_contacts.append(f_contact)
                    geom1s.append(contact_geom1)
    ...
    def get_world_cursor_pos(self, depth_img, fovy=45):
        ...
        cursor_pos_world = self.pixel_2_world(
            cursor_pos[0],
            cursor_pos[1],
            depth_img[cursor_pos[1], cursor_pos[0]],
            fovy=fovy,
        )
    ...
    def get_egocentric_rgb_depth_pcd(
        self,
        p_ego=None,
        p_trgt=None,
        rsz_rate=50,
        ...
        if (p_ego is not None) and (p_trgt is not None):
            ...
            self.update_viewer(
                ...
                lookat=cam_lookat,
                CALL_MUJOCO_FUNC=CALL_MUJOCO_FUNC,
            )
        ...
        if rsz_rate is not None:
            h_rsz, w_rsz = (
                depth_img.shape[0] // rsz_rate,
                depth_img.shape[1] // rsz_rate,
            )
    ...
    def get_egocentric_rgb_depth_pcd_offscreen(
        self,
        p_ego=None,
        p_trgt=None,
        rsz_rate=50,
        ...
        # Grab RGB and depth image
        rgb_img, depth_img = (
            self.grab_rgb_depth_img_offscreen()
    ...
    def execute_stability_verification(
        self,
        positions,
        target_object_name="obj_target_01",
        quat_lower_bound=0.70,
        quat_upper_bound=0.79,
        end_tick=1000,
        noise_tick=500,
        noise_scale=0.001,
        ...
        for position in positions:
            ...
            while (self.tick - start) < end_tick:
                ...
                if self.loop_every(HZ=20):
                    ...
                    self.plot_sphere(
                        ...
                        r=0.005,
                        rgba=[0, 1, 0, 1],
                        label=f"{self.tick}/{end_tick}",
                    )
                    self.plot_sphere(
                        p=position + np.array([0, 0, 0.1]),
                        r=0.005,
                        rgba=[0, 1, 0, 0],
                        label=f"Time: [{self.tick * 0.002:.4f}]",
                    )
                    self.plot_sphere(
                        p=position + np.array([0, 0, 0.2]),
                        r=0.005,
                        rgba=[0, 1, 0, 0],
                        label=f"Quaternion: [{r2quat(self.get_R_body(target_object_name))[0]:.4f}]",
                    )
                    [
                        self.plot_sphere(p=p___, r=0.005, rgba=[0, 1, 0, 1])
                        for p___ in downsample_pointcloud(
                            pointcloud=positions, grid_size=0.035
                        )
                    ]
            ...
            in_range = np.logical_and(
                np.array(R_target_list)[5:][:, 0] >= quat_lower_bound,
                np.array(R_target_list)[5:][:, 0] <= quat_upper_bound,
            )
            ...
            if all_in_range:
                ...
                R_feasible_range = [
                    np.max(np.array(R_target_list)[5:][:, 0])
                    - np.min(np.array(R_target_list)[5:][:, 0])
                ]
    ...
    def get_image_both_mode(self, body_name="ur_camera_center"):
        if self.MODE == "window":
            ...
            rgb_img, depth_img, pcd, xyz_img, xyz_img_world = (
                self.get_egocentric_rgb_depth_pcd(
                    p_ego=p_ego,
                    p_trgt=p_trgt,
                    rsz_rate=10,
                    fovy=45,
            ...
            scene_img_offscrenn, scene_depth_img_offscreen = (
                self.grab_rgb_depth_img_offscreen()
            )
            p_cam, R_cam = self.get_pR_body(body_name=body_name)
            p_ego = p_cam
            p_trgt = p_cam + R_cam[:, 2]
            rgb_img_offscrenn, depth_img_offscreen, pcd, xyz_img, xyz_img_world = (
                self.get_egocentric_rgb_depth_pcd_offscreen(
                    p_ego=p_ego,
                    p_trgt=p_trgt,
                    rsz_rate=10,
                    fovy=45,
    ...
    def move_object(
        ...
        target_position = (
            self.get_p_body(body_name=target_object_name)
            if isinstance(target_object_name, str)
    ...
    def get_closest_pairs(
        self, object_names: List[str], TOP_VIEW: bool, min_distance: float = 0.15
        ...
        closest_pairs_indices, closest_pairs_names = find_closest_objects_3d(
            object_positions, object_names, min_distance=min_distance
        )
        closest_obj_pairs_img = get_closest_pairs_img(
            object_positions, closest_pairs_indices, object_names, TOP_VIEW
        )
